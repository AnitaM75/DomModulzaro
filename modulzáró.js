...
const printCheaperThan = (price, products) => {
  const cheaperProducts = products.filter((product) => product.price < price);
...
const render = (products) => {
  ...
  products.forEach((product) => {
    ...
    detailItems.forEach((item) => detailsList.appendChild(item));
...
const uniqueCategories = getUniqueCategories(shopData);
...
const drawCategories = (categories) => {
  ...
  categories.forEach((category) => {
    ...
    button.addEventListener("click", () => {
      ...
      button.style.backgroundColor = activeButton !== button ? "red" : "";
      activeButton = activeButton !== button ? button : null;
    });
...
const render2 = products => {
    const productsContainer = document.getElementById('products');
    productsContainer.innerHTML = '';
    products.forEach(({ image, title, category, price, description }) => {
        productsContainer.innerHTML += `
            <div class="product-card">
...
const getUniqueCategories2 = products =>
    products.map(product => product.category)
            .filter((category, index, self) => self.indexOf(category) === index);

const drawCategoriesCollecting = categories => {
    ...
    if (!categoriesContainer) {
        ...
        categoriesContainer.className = 'categories-container';
        const productsContainer = document.getElementById('products');
        productsContainer.parentNode.insertBefore(categoriesContainer, productsContainer);
    }
    ...
    categories.forEach(category => {
        ...
        button.addEventListener('click', () => {
            ...
            button.style.backgroundColor = (activeButton !== button) ? 'red' : '';
            activeButton = (activeButton !== button) ? button : null;
            render2(activeButton ? shopData.filter(product => product.category === button.textContent) : shopData);
        });

        categoriesContainer.appendChild(button);
    });
};
const uniqueCategories2 = getUniqueCategories2(shopData);
drawCategoriesCollecting(uniqueCategories2);
render2(shopData);
